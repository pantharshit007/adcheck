...
(() => {
    ...
    async function bootstrap() {
        ...
        chrome.storage.onChanged.addListener((changes, areaName) => {
            ...
            const merged = AdCheckShared.mergeSettings(changes[AdCheckShared.STORAGE_KEY].newValue);
    ...
    async function runChecks(resetDeadline) {
    ...
    async function syncNetworkAndRefresh(resetDeadline) {
        state.networkState = await getNetworkState(resetDeadline ? "REFRESH_TAB_NETWORK_STATE" : "GET_TAB_NETWORK_STATE");
        state.snapshot = buildSnapshot();
        renderWidget();
    }
    ...
    function buildClassResults() {
        return state.settings.classNames.map((className) => {
            ...
            return {
                ...
                failureMessage: status === "fail" ? "Class name not found - the expected ad markup may not have rendered." : undefined
            };
        });
    }
    function buildDomResults() {
    ...
    function buildAttributeResults() {
            ...
            return {
                ...
                detail: value !== undefined
                    ...
                    : status === "pending"
                        ...
                        : "Cookie not found.",
                failureMessage: status === "fail" ? "Cookie not found - the page may be missing required ad identity data." : undefined,
                storageKind: "cookie",
                valuePreview: value
            };
        });
    }
    function buildLocalStorageResults() {
        return state.settings.localStorageKeys.map((storageKey) => {
            ...
            return {
                ...
                failureMessage: status === "fail" ? "Local storage key not found - the page may not have saved the expected ad state." : undefined,
    ...
    function renderWidget() {
        ...
        const widget = state.root.querySelector(".adcheck-widget");
        const results = state.root.querySelector("#adcheckResults");
        const badge = state.root.querySelector("#adcheckStatusBadge");
        const toggleLabel = state.root.querySelector("#adcheckToggleTab span");
    ...
    function renderGroup(title, description, results) {
    ...
    function bindWidgetEvents() {
        ...
        state.root.querySelector("#adcheckRefreshButton")?.addEventListener("click", () => {
            void runChecks(true);
        });
        state.root.querySelector("#adcheckHideButton")?.addEventListener("click", () => {
            void persistCollapsedState(true);
        });
        state.root.querySelector("#adcheckToggleTab")?.addEventListener("click", () => {
    ...
    function bindResultEvents() {
        ...
        for (const button of Array.from(state.root.querySelectorAll("[data-dom-id]"))) {
            button.addEventListener("click", (event) => {
                event.stopPropagation();
                void navigateToDomId(button.dataset.domId ?? "");
            });
        }
        for (const row of Array.from(state.root.querySelectorAll("[data-dom-result]"))) {
            row.addEventListener("click", () => {
                void navigateToDomId(row.dataset.domResult ?? "");
            });
        }
    }
    async function navigateToDomId(domId) {
        if (!domId) {
            return;
        }
    ...
    function setRowHint(key, message) {
    ...
    async function persistCollapsedState(nextCollapsed) {
        const merged = {
            ...state.settings,
    ...
    function scheduleDeadlineRender() {
        if (state.deadlineHandle) {
            window.clearTimeout(state.deadlineHandle);
        }
    ...
    function pendingOrFailedStatus() {
    ...
    function countTotalChecks() {
        return (state.snapshot.bundles.length +
    ...
    async function getNetworkState(messageType) {
        const response = await sendMessage({
            type: messageType
        });
