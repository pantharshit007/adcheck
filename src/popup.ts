...
(() => {
  ...
  const popupStateLabel = document.getElementById(
    "popupStateLabel",
  ) as HTMLParagraphElement | null;
  ...
  const importExportEditor = document.getElementById(
    "importExportEditor",
  ) as HTMLDivElement | null;
  ...
  const siteOverrideEditor = document.getElementById(
    "siteOverrideEditor",
  ) as HTMLDivElement | null;
  ...
  async function initializePopup(): Promise<void> {
    ...
    siteOverrideSnippetInput.addEventListener("input", () => {
      renderInlineScriptPermissionWarning(
        hasInlineScript(siteOverrideSnippetInput.value),
      );
    });
  ...
  async function initializeSiteOverridePanel(): Promise<void> {
    ...
    popupState.userScriptStatus = await loadUserScriptStatus();
    ...
    popupState.pickedSelection = await loadSiteSelection(
      popupState.activeSite.hostname,
    );
  ...
  function renderSiteOverrideUnavailable(): void {
    ...
    siteOverrideSelectionLabel.textContent =
      "Site overrides are unavailable on this tab.";
    renderInlineScriptPermissionWarning(false);
  ...
  function hydrateSiteOverridePanel(
    ...
    renderPickedSelection(
      selection ?? selectorToSelection(override?.selector ?? ""),
    );
    setSiteOverrideEditorVisible(false); // always start collapsed, like other sections
    renderInlineScriptPermissionWarning(
      hasInlineScript(override?.htmlSnippet ?? ""),
    );
  }
  ...
  function renderPickedSelection(selection: SitePickerSelection | null): void {
    ...
    const dimensions = selection.dimensionsLabel
      ? ` (${selection.dimensionsLabel})`
      : "";
  ...
  async function startSiteElementPicker(): Promise<void> {
    ...
    try {
      ...
      showStatus(
        "Picker is armed. Click the page element, then reopen AdCheck.",
      );
      window.close();
    } catch {
  ...
  async function saveSiteOverride(): Promise<void> {
    ...
    renderInlineScriptPermissionWarning(hasInlineScript(htmlSnippet));

    const nextOverride: SiteOverrideRule = {
      hostname: popupState.activeSite.hostname,
      selector,
      placement: AdCheckShared.normalizePlacement(
        siteOverridePlacementSelect.value,
      ),
      htmlSnippet,
    ...
    showStatus(
      hasInlineScript(htmlSnippet) &&
        popupState.userScriptStatus?.available === false
  ...
  async function loadSiteOverrides(): Promise<SiteOverrideRule[]> {
    const result = await chrome.storage.local.get(
      AdCheckShared.SITE_OVERRIDE_STORAGE_KEY,
    );
    return AdCheckShared.normalizeSiteOverrides(
      result[AdCheckShared.SITE_OVERRIDE_STORAGE_KEY] as
        | SiteOverrideRule[]
        | undefined,
    );
  }

  async function loadSiteOverride(
    hostname: string,
  ): Promise<SiteOverrideRule | null> {
    const overrides = await loadSiteOverrides();
    return AdCheckShared.findSiteOverrideForHostname(overrides, hostname);
  }

  async function loadSiteSelection(
    hostname: string,
  ): Promise<SitePickerSelection | null> {
    const result = await chrome.storage.local.get(
      AdCheckShared.sitePickSelectionStorageKey(hostname),
    );
    const selection = result[
      AdCheckShared.sitePickSelectionStorageKey(hostname)
    ] as SitePickerSelection | undefined;
  ...
  async function loadUserScriptStatus(): Promise<UserScriptStatus | null> {
    try {
      const response = await chrome.runtime.sendMessage({
        type: "GET_USER_SCRIPT_STATUS",
      } satisfies AdCheckShared.RuntimeMessage);
      return (response?.status as UserScriptStatus | undefined) ?? null;
    } catch {
      return null;
    }
  }

  function renderInlineScriptPermissionWarning(
    hasInlineScriptSnippet: boolean,
  ): void {
    ...
    const needsPermissionWarning =
      hasInlineScriptSnippet &&
      popupState.userScriptStatus?.available === false &&
      popupState.userScriptStatus.message;
    siteOverridePermissionMessage.classList.toggle(
      "is-hidden",
      !needsPermissionWarning,
    );
    siteOverridePermissionMessage.textContent = needsPermissionWarning
      ? (popupState.userScriptStatus?.message ?? "")
      : "";
  }
  ...
  function renderSettingsForm(settings: Settings): void {
    ...
    popupStateLabel.textContent = settings.enabled
      ? "AdCheck Active"
      : "AdCheck Paused";
  ...
  function renderSection(section: SettingsSection, settings: Settings): string {
    ...
    const countBadge =
      itemCount > 0
        ? `<span class="adcheck-section-count">${itemCount}</span>`
        : "";
  ...
  function toggleSection(sectionKey: string): void {
    if (!formRoot) return;
    const section = formRoot.querySelector<HTMLElement>(
  ...
  function addRow(sectionKey: string): void {
    ...
    const list = formRoot.querySelector<HTMLElement>(
      `[data-entry-list="${sectionKey}"]`,
    );
    if (!list) {
      return;
    ...
    const section = AdCheckShared.SETTINGS_SECTIONS.find(
    ...
    wrapper
      .querySelector<HTMLButtonElement>("[data-remove-row]")
      ?.addEventListener("click", () => {
        removeRow(
          wrapper.querySelector<HTMLButtonElement>("[data-remove-row]"),
        );
      });
  }
  ...
  async function persistEnabledState(): Promise<void> {
    ...
    await persistSettings(
      settings,
      settings.enabled ? "AdCheck enabled." : "AdCheck disabled.",
    );
  }
    ...
    for (const section of AdCheckShared.SETTINGS_SECTIONS) {
      const inputs = Array.from(
        document.querySelectorAll<HTMLInputElement>(
          `input[data-section-key="${section.key}"]`,
        ),
      );
  ...
  function parseImportedSettings(value: string): Settings | null {
    ...
    try {
      return AdCheckShared.mergeSettings(
        JSON.parse(trimmed) as Partial<Settings>,
      );
    } catch {
      return null;
    }
  ...
  function setImportEditorVisible(
    isVisible: boolean,
    focusEditor = false,
  ): void {
    ...
    toggleImportEditorButton.textContent = isVisible
      ? "Hide editor"
      : "Show editor";
  ...
  function setSiteOverrideEditorVisible(isVisible: boolean): void {
    ...
    toggleSiteOverrideEditorButton.textContent = isVisible
      ? "Hide details"
      : "Show details";
  }
